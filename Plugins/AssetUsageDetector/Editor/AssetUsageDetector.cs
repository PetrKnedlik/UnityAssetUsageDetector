...
namespace AssetUsageDetectorNamespace
{
    ...
    public partial class AssetUsageDetector
    {
        ...
        private readonly Dictionary<string, ReferenceNode> searchedObjects = new Dictionary<
            string,
            ReferenceNode
        >(4096);
        private readonly Dictionary<int, ReferenceNode> searchedUnityObjects = new Dictionary<
            int,
            ReferenceNode
        >(32768); // Unity objects use their instanceIDs as key which is more performant
        ...
        public SearchResult Run(Parameters searchParameters)
        {
            ...
            if (openPrefabStage != null)
            {
                ...
                else
                {
                    ...
                    openPrefabStagePrefabAsset = AssetDatabase.LoadAssetAtPath<GameObject>(
                        prefabAssetPath
                    );
            ...
            if (
                !isInPlayMode
                && !Utilities.AreScenesSaved()
                && !EditorUtility.DisplayDialog(
                    "Asset Usage Detector",
                    "Some scene(s) aren't saved. This may result in incorrect search results in those scene(s). Proceed?",
                    "Yes",
                    "Cancel"
                )
            )
            ...
            SceneSetup[] initialSceneSetup = !isInPlayMode
            ...
            try
            {
                ...
                if (searchParameters.searchInProjectSettings && assetsToSearchSet.Count > 0)
                {
                    ...
                    for (int i = 0; i < projectSettingsAssets.Length; i++)
                        projectSettingsToSearch[i] =
                            "ProjectSettings/" + Path.GetFileName(projectSettingsAssets[i]);
                ...
                if (
                    ...
                    && searchParameters.searchInScenesSubset.Length > 0
                )
                {
                    foreach (Object obj in searchParameters.searchInScenesSubset)
                    {
                        ...
                        if (obj.IsFolder())
                        {
                            string[] folderContents = AssetDatabase.FindAssets(
                                "t:SceneAsset",
                                new string[] { AssetDatabase.GetAssetPath(obj) }
                            );
                            ...
                            for (int i = 0; i < folderContents.Length; i++)
                                scenesToSearch.Add(
                                    AssetDatabase.GUIDToAssetPath(folderContents[i])
                                );
                        }
                    ...
                    (searchParameters.searchInScenes & SceneSearchMode.AllScenes)
                ...
                {
                    if (
                        (searchParameters.searchInScenes & SceneSearchMode.OpenScenes)
                    ...
                    bool searchInScenesInBuildTickedAll =
                        (searchParameters.searchInScenes & SceneSearchMode.ScenesInBuildSettingsAll)
                        == SceneSearchMode.ScenesInBuildSettingsAll;
                    if (
                        searchInScenesInBuildTickedAll
                        || (
                            searchParameters.searchInScenes
                ...
                if (isInPlayMode)
                {
                    ...
                    if (skippedScenes.Count > 0)
                    {
                        ...
                        sb.Length = 0;
                        sb.Append("Can't search unloaded scenes while in play mode, skipped ")
                            .Append(skippedScenes.Count)
                            .AppendLine(" scene(s):");
                ...
                {
                    currentSearchResultGroup = new SearchResultGroup(
                        "Project Window (Assets)",
                        SearchResultGroup.GroupType.Assets
                    );
                    ...
                    if (
                        ...
                        || searchParameters.searchInAssetsSubset.Length == 0
                    )
                    {
                    ...
                    if (searchParameters.excludedAssetsFromSearch != null)
                    {
                        foreach (Object obj in searchParameters.excludedAssetsFromSearch)
                        {
                            ...
                            if (obj.IsFolder())
                                excludedAssetsPathsSet.UnionWith(
                                    Utilities.EnumerateFolderContents(obj)
                                );
                            else
                    ...
                    {
                        if (
                            searchParameters.showDetailedProgressBar
                            && ++searchProgress % 30 == 1
                            && EditorUtility.DisplayCancelableProgressBar(
                                "Please wait...",
                                "Searching assets",
                ...
                if (projectSettingsToSearch.Length > 0)
                {
                    currentSearchResultGroup = new SearchResultGroup(
                        "Project Settings",
                        SearchResultGroup.GroupType.ProjectSettings
                    );

                    if (
                        EditorUtility.DisplayCancelableProgressBar(
                            "Please wait...",
                            "Searching Project Settings",
                            (float)searchProgress / searchTotalProgress
                        )
                    )
                        throw new Exception("Search aborted");
                ...
                if (scenesToSearch.Count > 0)
                {
                    ...
                    foreach (string scenePath in scenesToSearch)
                    {
                        ...
                        {
                            var packageInfo =
                                UnityEditor.PackageManager.PackageInfo.FindForAssetPath(scenePath);
                            if (
                                packageInfo != null
                                && packageInfo.source
                ...
                if (isInPlayMode && searchParameters.searchInScenes != SceneSearchMode.None)
                {
                    if (
                        EditorUtility.DisplayCancelableProgressBar(
                            "Please wait...",
                            "Searching scene: DontDestroyOnLoad",
                            1f
                        )
                    )
                        throw new Exception("Search aborted");
                    ...
                    currentSearchResultGroup = new SearchResultGroup(
                        "DontDestroyOnLoad",
                        SearchResultGroup.GroupType.DontDestroyOnLoad
                    );
                ...
                Debug.Log(
                    "Searched "
                        ...
                        + " objects in "
                        + (EditorApplication.timeSinceStartup - searchStartTime).ToString("F2")
                        + " seconds"
                );
            ...
            catch (Exception e)
            {
                ...
                sb.AppendLine(
                    "<b>AssetUsageDetector Error:</b> The following Exception is thrown during the search. Details:"
                );
            ...
            finally
            {
                ...
                if (!string.IsNullOrEmpty(openPrefabStageAssetPath))
                {
                    ...
                    if (
                        openPrefabStageContextObject != null
                        && !openPrefabStageContextObject.Equals(null)
                    )
                    {
                        try
                        {
                            ...
                            MethodInfo prefabStageOpenerWithContext =
                                typeof(PrefabStageUtility).GetMethod(
                                    ...
                                    BindingFlags.Public
                                        ...
                                        | BindingFlags.Static,
                                    null,
                                    new Type[2] { typeof(string), typeof(GameObject) },
                                    null
                                );
                            if (prefabStageOpenerWithContext != null)
                            {
                                prefabStageOpenerWithContext.Invoke(
                                    null,
                                    new object[2]
                                    {
                                        openPrefabStageAssetPath,
                                        openPrefabStageContextObject,
                                    }
                                );
                    ...
                    if (shouldOpenPrefabStageWithoutContext)
                        AssetDatabase.OpenAsset(
                            AssetDatabase.LoadAssetAtPath<GameObject>(openPrefabStageAssetPath)
                        );
                }
            }
        }
        ...
        private void CalculateUnusedObjects(
            List<SearchResultGroup> searchResult,
            out HashSet<Object> usedObjectsSet
        )
        {
            currentSearchResultGroup = new SearchResultGroup(
                "Unused Objects",
                SearchResultGroup.GroupType.UnusedObjects,
                false,
                false
            );
            ...
            foreach (SearchResultGroup searchResultGroup in searchResult)
            {
                for (int j = 0; j < searchResultGroup.NumberOfReferences; j++)
                {
                    ...
                    if (usedObjectsSet.Add(obj))
                    {
                        ...
                        {
                            for (
                                Transform parent = ((GameObject)obj).transform.parent;
                                parent != null;
                                parent = parent.parent
                            )
                                usedObjectPathsSet.Add(
                                    parent.gameObject.GetInstanceID().ToString()
                                );
                        }
                    }
                }
            ...
            Dictionary<string, ReferenceNode> unusedMainObjectNodes = new Dictionary<
                string,
                ReferenceNode
            >(objectsToSearchSet.Count - usedObjectsSet.Count);
            Dictionary<string, List<ReferenceNode>> unusedSubObjectNodes = new Dictionary<
                string,
                List<ReferenceNode>
            >(objectsToSearchSet.Count - usedObjectsSet.Count);
            foreach (Object obj in objectsToSearchSet)
            {
                ...
                if (
                    AssetUsageDetectorSettings.MarkUsedAssetsSubAssetsAsUsed
                    && AssetDatabase.IsSubAsset(obj)
                    && usedObjectsSet.Contains(AssetDatabase.LoadMainAssetAtPath(assetPath))
                )
                    continue;
                ...
                if (obj is GameObject)
                {
                    if (string.IsNullOrEmpty(assetPath))
                    {
                        ...
                        {
                            if (
                                objectsToSearchSet.Contains(parent)
                                && !usedObjectsSet.Contains(parent.gameObject)
                            )
                ...
                if (string.IsNullOrEmpty(assetPath))
                {
                    ...
                    else // List child GameObject scene objects under their parent GameObject
                    {
                        ...
                        if (
                            !unusedSubObjectNodes.TryGetValue(
                                dictionaryKey,
                                out unusedSubObjectNodesAtPath
                            )
                        )
                            unusedSubObjectNodes[dictionaryKey] = unusedSubObjectNodesAtPath =
                                new List<ReferenceNode>(2);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (
                            ...
                            unusedSubObjectNodes[assetPath] = unusedSubObjectNodesAtPath =
                                new List<ReferenceNode>(2);
        ...
        )
        {
            ...
            bool canContainSceneObjectReference =
                scene.isLoaded
                && (
            ...
            {
                bool canContainAssetReference =
                    assetsToSearchSet.Count > 0
                    && (isInPlayMode || AssetHasAnyReference(scenePath));
                ...
                if (searchParameters.lazySceneSearch)
                {
                    searchResult.Add(
                        new SearchResultGroup(
                            scenePath,
                            SearchResultGroup.GroupType.Scene,
                            true,
                            true
                        )
                    );
            ...
            if (
                searchParameters.searchInSceneLightingSettings
                && (!isInPlayMode || SceneManager.GetActiveScene() == scene)
            )
            {
        ...
        private ReferenceNode SearchObject(object obj)
        {
            ...
            bool searchingSourceAsset =
                searchingSourceAssets && ReferenceEquals(currentSearchedObject, obj);
            ...
            if (unityObject != null)
            {
                ...
                if (unityObject.IsAsset())
                {
                    ...
                    if (
                        excludedAssetsPathsSet.Contains(assetPath)
                        || !AssetHasAnyReference(assetPath)
                    )
                    {
                        searchedUnityObjects.Add(unityObject.GetInstanceID(), null);
                        return null;
                ...
                if (
                    assetPath != null
                    && extensionToSearchFunction.TryGetValue(
                        Utilities.GetFileExtension(assetPath),
                        out searchFunction
                    )
                    ...
                    typeToSearchFunction.TryGetValue(unityObject.GetType(), out searchFunction)
                )
                    result = searchFunction(unityObject);
                ...
                )
                {
                    ...
                    Transform[] prefabChildren =
                        prefabGameObject.GetComponentsInChildren<Transform>(true);
                    for (int i = 0; i < prefabChildren.Length; i++)
                    {
                        if (prefabChildren[i].gameObject != prefabGameObject)
                            result.AddLinkTo(
                                SearchObject(prefabChildren[i].gameObject),
                                isWeakLink: true
                            );
                    }
                }
        ...
        private bool AssetHasAnyReference(string assetPath)
        {
            ...
            if (
                alwaysSearchedExtensionsSet.Count > 0
                && alwaysSearchedExtensionsSet.Contains(Utilities.GetFileExtension(assetPath))
            )
                return true;
                ...
                if (!Directory.Exists(dependencies[i])) // Calling FileInfo.Length on a directory throws FileNotFoundException
                {
                    ...
                    if (!assetFile.Exists || assetFile.Length != fileSizes[i])
                    {
                        ...
                        if (lastRefreshedCacheEntry == cacheEntry)
                        {
                            ...
                            sb.AppendLine(
                                    "<b>Infinite loop while refreshing a cache entry, please report it to the author.</b>"
                                )
                                .AppendLine();
                            ...
                            for (int j = 0; j < 2; j++)
                            {
                                ...
                                sb.AppendLine()
                                    .AppendLine(j == 0 ? "Old Dependencies:" : "New Dependencies");
        ...
        private bool TryGetReferenceNode(object nodeObject, out ReferenceNode referenceNode)
        {
            ...
            {
                if (
                    searchedUnityObjects.TryGetValue(
                        ((Object)nodeObject).GetInstanceID(),
                        out referenceNode
                    )
                )
                    return true;
            ...
            if (callStack.Count > 0)
            {
                ...
                for (int i = callStack.Count - 1; i >= 0; i--)
                {
                    ...
                    if (latestUnityObjectInCallStack)
                    {
                        ...
                        {
                            string scenePath = AssetDatabase.GetAssetOrScenePath(
                                latestUnityObjectInCallStack
                            );
                            if (
                ...
                for (int i = callStack.Count - 1; i >= 0; i--)
                {
                    ...
                    if (unityObject)
                        sb.Append(unityObject.name)
                            .Append(" (")
                            .Append(unityObject.GetType())
                            .AppendLine(")");
