...
namespace AssetUsageDetectorNamespace
{
    public abstract class ListDrawer<T>
    {
        ...
        public bool Draw(List<T> list)
        {
            ...
            if (guiEnabled)
            {
                ...
                if (
                    (ev.type == EventType.DragPerform || ev.type == EventType.DragUpdated)
    ...
    {
        public ObjectListDrawer(string label, bool acceptSceneObjects)
            : base(label, acceptSceneObjects) { }
    ...
    {
        public ObjectToSearchListDrawer()
            : base("Find references of:", true) { }
        ...
        protected override void SetObjectOfElement(
            List<ObjectToSearch> list,
            int index,
            Object value
        )
        {
        ...
        protected override void PostElementDrawer(ObjectToSearch element)
        {
            ...
            if (subAssetsToSearch.Count > 0)
            {
                ...
                GUI.changed = false;
                toggleAllSubAssets = EditorGUILayout.Toggle(
                    toggleAllSubAssets,
                    Utilities.GL_WIDTH_25
                );
                if (GUI.changed)
                {
                ...
                element.showSubAssetsFoldout = EditorGUILayout.Foldout(
                    element.showSubAssetsFoldout,
                    "Include sub-assets in search:",
                    true
                );
                ...
                if (element.showSubAssetsFoldout)
                {
                    for (int j = 0; j < subAssetsToSearch.Count; j++)
                    {
                        ...
                        subAssetsToSearch[j].shouldSearch = EditorGUILayout.Toggle(
                            subAssetsToSearch[j].shouldSearch,
                            Utilities.GL_WIDTH_25
                        );
